...
def batch_modify_header(bucket,object):
    ...
    flag = 1
    while flag:
        header_input = raw_input(u"请输入header选项:['Content-Type','Cache-Control','Content-Disposition','Content-Encoding','Content-Language','Expires'] or 输入保存后退出[save]:".encode('mbcs')).title()
        if header_input == 'Content-Type':
            ...
            content_encoding = raw_input(u'请输入需要设置的Content-Encoding值:'.encode('mbcs'))
            headers['Content-Encoding'] = content_encoding
        elif header_input == 'Content-Language':
            content_language = raw_input(u'请输入需要设置的Content-Language值:'.encode('mbcs'))
    ...
    print u"您需要修改的主机头参数为 %s" %headers
...
def choice_list():
    while 1:
        ...
        if choice == '1':
            ...
            bucket = raw_input(u'请输入需要统计的bucket名称:'.encode('mbcs'))
            sum_bucket_size(bucket)
        elif choice == '3':
            path = raw_input(u'请输入需要修改主机头的bucket路径:'.encode('mbcs'))
            (bucket, object) = parse_bucket_object(path)
            ...
            localpath = raw_input(u'请输入需要批量上传文件所在的目录路径:'.encode('mbcs'))
            path = raw_input(u'请输入需要上传至bucket的名称路径:'.encode('mbcs'))
            upload_object_from_localdir(check_point_file,localpath,path)
        elif choice  == '6':
            source_bucket = raw_input(u'请输入需要迁移的源bucket名称:'.encode('mbcs'))
            source_bucket_location = oss.get_bucket_location(source_bucket)
            print source_bucket_location.read()
            target_bucket = raw_input(u'请输入需要迁移的目的bucket名称:'.encode('mbcs'))
            ...
            bucket = raw_input(u'请输入需要下载文件所在的bucket路径:'.encode('mbcs'))
            local_path = raw_input(u'请输入文件保存的路径:'.encode('mbcs'))
